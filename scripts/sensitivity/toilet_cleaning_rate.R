...
if (run_sims) {
  sens_toilet_cleaning_rate <- progressr::with_progress(
    run_nl_all(nl))
  write_rds(sens_toilet_cleaning_rate, file.path(out_path, "sens_toilet_cleaning_rate.rds"))
}

sens_toilet_cleaning_rate <- read_rds(file.path(out_path, "sens_toilet_cleaning_rate.rds"))

sens_toilet_cleaning_rate_bak <- sens_toilet_cleaning_rate

sens_toilet_cleaning_rate <- sens_toilet_cleaning_rate %>% 
  filter(`[step]` > 31)
...
sens_toilet_cleaning_rate_rates <- sens_toilet_cleaning_rate %>% 
...
sens_toilet_cleaning_rate_outbreak_rates <- sens_toilet_cleaning_rate %>% 
...
sens_toilet_cleaning_rate_outbreak_rates %>% 
...
sens_toilet_cleaning_rate_outbreak_cases <- sens_toilet_cleaning_rate %>% 
  filter(`[step]` >= filter_from & `[step]` <= filter_to) %>% 
  mutate(year = year(date_sim), month = month(date_sim)) %>%
...
sens_toilet_cleaning_rate_outbreak_cases %>% 
  group_by(`c-toilet-cleaning-rate`) %>% 
...
list(abx_prescription_rate = sens_abx_prescription_outbreak_cases,
     proportion_redistributed = sens_toilet_cleaning_rate_outbreak_cases) %>%
  bind_rows() %>% 
