# The initial sensitivity analysis suggested that isolating no patients was better
# than isolating a few patients (in terms of reducing the number of cases). This
# is counterintuitive but probably because if no patients are isolated then it
# avoids contaminating side room toilets.

# To test this, this runs the same sensitivity analysis but with the toilet cleaning
# effect 100% effective.
...
estimated_consts_mean["toilet-cleaning-effect"] <- 1
estimated_consts_mean["o-toilet-cleaning-effect"] <- 1
...
if (run_sims) {
  sens_prop_redist_perfect_clean <- progressr::with_progress(
    run_nl_all(nl))
  write_rds(sens_prop_redist_perfect_clean, file.path(out_path, "sens_prop_redist_perfect_clean.rds"))
}

sens_prop_redist_perfect_clean <- read_rds(file.path(out_path, "sens_prop_redist_perfect_clean.rds"))

sens_prop_redist_perfect_clean_bak <- sens_prop_redist_perfect_clean

sens_prop_redist_perfect_clean <- sens_prop_redist_perfect_clean %>% 
  filter(`[step]` > 31)
...
list(abx_prescription_rate = sens_abx_prescription_outbreak_cases,
     proportion_redistributed = sens_prop_redist_perfect_clean_outbreak_cases) %>%
  bind_rows() %>% 
