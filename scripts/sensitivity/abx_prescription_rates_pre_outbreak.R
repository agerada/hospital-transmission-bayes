...
estimated_consts_mean <- estimated_consts_mean[names(estimated_consts_mean) != "antibiotic-prescription-rate"]
estimated_consts_mean <- estimated_consts_mean[names(estimated_consts_mean) != "o-antibiotic-prescription-rate"]
...
nl@experiment <- experiment(expname = "sens_analysis",
                            ...
                            variables = list("antibiotic-prescription-rate" = list(values = seq(from=0.01, to=0.99, length.out = sens_samples)),
                                             "o-antibiotic-prescription-rate" = list(values = seq(from=0.01, to=0.99, length.out = sens_samples))
                            )
)
...
if (run_sims) {
  sens_abx_prescription_pre_outbreak <- progressr::with_progress(
    run_nl_all(nl))
  write_rds(sens_abx_prescription_pre_outbreak, file.path(out_path, "sens_abx_prescription_pre_outbreak.rds"))
}

sens_abx_prescription_pre_outbreak <- read_rds(file.path(out_path, "sens_abx_prescription_pre_outbreak.rds"))

sens_abx_prescription_pre_outbreak_bak <- sens_abx_prescription_pre_outbreak

sens_abx_prescription_pre_outbreak <- sens_abx_prescription_pre_outbreak %>% 
  filter(`[step]` > 31)
...
sens_abx_prescription_pre_outbreak_rates <- sens_abx_prescription_pre_outbreak %>% 
  mutate(year = year(date_sim), month = month(date_sim)) %>% 
  group_by(`random-seed`, `antibiotic-prescription-rate`, year, month) %>% 
...
sens_abx_prescription_pre_outbreak_outbreak_rates <- sens_abx_prescription_pre_outbreak %>% 
...
sens_abx_prescription_pre_outbreak_outbreak_rates %>% 
  group_by(`antibiotic-prescription-rate`, `random-seed`) %>% 
  summarise(rate = mean(rate)) %>%
  group_by(`antibiotic-prescription-rate`) %>%
  summarise(mean_rate = mean(rate),
            sd_rate = sd(rate)) %>% 
  ggplot(aes(x = `antibiotic-prescription-rate`, y = mean_rate)) +
  geom_line() + 
  # add sd
  geom_line(data = . %>% 
...
sens_abx_prescription_pre_outbreak_outbreak_cases <- sens_abx_prescription_pre_outbreak %>% 
  filter(`[step]` >= filter_from & `[step]` <= filter_to) %>% 
  mutate(year = year(date_sim), month = month(date_sim)) %>%
  group_by(`random-seed`, `antibiotic-prescription-rate`) %>% 
