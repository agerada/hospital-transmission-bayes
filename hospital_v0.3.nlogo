...
to update-patients
  ...
  ask patients [ if ticks - admission-tick > random-poisson ( admission-days * 24 )
    [
  ...
  ask patients [ if random-float ( antibiotic-effect * ( ifelse-value antibiotic-treated? [ 1 ] [ 0 ] ) ) > random-colonisation-thresh
    [
      set colonised? true
      set hospital-infection? true
      set total-hospital-infections total-hospital-infections + 1
...
to contaminate-toilet [ t ]
  ask t [ if colonised? = true [set toilet-contamination toilet-contamination + random-float toilet-contamination-effect ] ]
end
...
"" ""
PENS
"Total admitted" 1.0 0 -11085214 true "" "plot total-patients-admitted"
"Total colonised" 1.0 0 -2674135 true "" "plot total-colonised"
