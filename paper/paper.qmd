---
title: Gastro-intestinal transmission of _Clostridioides difficile_ and antimicrobial-resistant bacteria in healthcare settings -- calibration of an agent-based model
authors:
...
\acr[case=sentence]{IPC} measures are designed to prevent the spread of these pathogens within healthcare settings. Commonly used interventions include hand hygiene, environmental cleaning, and the isolation of infected patients [@worldhealthorganizationGlobalReportInfection2022].
...
-   $m$ is the increased (or decreased) relative risk of infection due to antimicrobial therapy,
...
-   $p$ is the community infection rate,
...
```{r fig-sd-model, include=TRUE}
#| echo: false
#| fig.cap: "System dynamics model. $S$ and $I$ are the number of susceptible and infected patients, respectively; $A$ and $D$ are the rates of admitted and discharged patients, respectively; $p$ is the community infection rate; $\\beta$ is the transmission coefficient; and $r$ is the rate of random (*de novo*) infection."
knitr::include_graphics("./images/sd_model.svg")
```
...
$$
D_{S,t} = \left \lfloor D_{\text{total}, t} \cdot \frac{S_t}{S_t + I_t} \right \rceil,
$$

and

$$
D_{I,t} = \left \lfloor D_{\text{total}, t} \cdot \frac{I_t}{S_t + I_t} \right \rceil = D_{\text{total}, t} - D_{S,t},,
$$
...
A similar approach was used to generate outbreaks in the stochastic system dynamics model, with the difference that the parameters were changed at: 1) onset of outbreak; and 2) end of outbreak. The parameters that were changed were: 1) $\beta$ (transmission coefficient); and 2) $p$ (community infection rate). The parameter $r$ (rate of random infection) was kept constant throughout the simulation. 
...
### Calibration of \acr[first_use=TRUE]{ABM}
...
### Calibration of \acr[first_use=TRUE]{SD} model

\acr[first_use=TRUE, case=sentence]{ABC} was also used to calibrate the stochastic \acr{SD} model to the observed outbreak data, although the calibration was performed in a single step that encompassed the entire period of the observed data (i.e., pre-outbreak, outbreak, and control periods). 
...
Both the \acr{ABM} and the \acr{SD} model were able to simulate the faecal-oral transmission of infection in a hospital setting. Although the \acr{SD} was computationally simpler, and produced simulated infection rates that were very similar to the observed rates (see @fig-posterior-predictive-check), the \acr{ABM} was based on a simulation that more closely resembled the workings of a real hospital. Importantly, the parameters of the \acr{ABM} were directly based on modifiable infection control practices, such as toilet cleaning frequency and antimicrobial prescription rates. Therefore, the main analyses and senstivity analyses were performed using the \acr{ABM}.
...
_The discussion needs to explore the different modelling approaches used (SD vs ABM) and why we eventually settled on the ABM as the main model. This is sort of captured in the results, but maybe it needs to be here as well?_
