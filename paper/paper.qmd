...
-   $P_i(t)$ is the risk of patient $i$ acquiring infection at time $t$,
...
In the agent-based model implementation, Bernoulli trials were conducted to determine if a patient became infected at each time step $t$:

$$
X_\text{de novo, i}(t) \sim \text{Bernoulli}(P_{\text{de novo}, i}(t)),
$$

where $X_{\text{de novo}, i}$ = 1 if patient $i$ becomes infected *de novo* at time $t$, and 0 otherwise.

Further Bernoulli trials were conducted to determine if a patient became infected from a contaminated toilet at each time step $t$:

$$
X_{\text{toilet}, i}(t) \sim \text{Bernoulli}(P_{\text{toilet}, i}(t)),
$$

where $X_{\text{toilet}, i}$ = 1 if patient $i$ becomes infected from a contaminated toilet at time $t$, and 0 otherwise.

The results of the trials were combined to determine the outcome for patient $p$ at time $t$; the patient became infected if either trial was successful:

$$
X_i(t) = \text{Max}(X_{\text{de novo}, i}, X_{\text{toilet}, i}),
$$

where $X_i(t)$ = 1 if patient $i$ becomes infected at time $t$, and 0 otherwise.
...
Each patient's admission duration was sampled from a Poisson distribution with a configurable mean (in days). Upon discharge, patients were removed from the simulation, and replaced with new admissions to maintain 100% occupancy. New admissions had a probability of being infected based on the community infection rate parameter.

## Stochastic system dynamics model

To build a high-level stochastic system dynamics model, the model was first described in terms of the following differential equations (see @fig-sd-model), an adaptation of *SI* (susceptible--infection) models [@andersonDiscussionKermackMcKendrickEpidemic1991; @keelingModelingInfectiousDiseases2011]:

$$
\frac{dS}{dt} = (1-\lambda)A - D \cdot \frac{S}{S + I} - S (\beta + r)
$$

$$
\frac{dI}{dt} = \lambda A - D \cdot \frac{I}{S + I} + S (\beta + r)
$$
...
-   $dS/dt$ and $dI/dt$ are the rates of change of susceptible/non-infected and infected inpatients, respectively,
-   $A$ is the number of new admissions,
-   $D$ is the number of patient discharges,
-   $\lambda$ is the community infection rate,
...
Next, the number of susceptible and infected patients at each discrete time $t + 1$ was calculated by first determining the number of patients discharged:

$$
D_{\text{total}, t} \sim \text{Poisson}(d),
$$

where $d$ is the discharge rate and $D_{\text{total}, t}$ is the total number of patients (both susceptible, $D_S$, and infected $D_I$), discharged at time $t$.
...
$$
A_{I,t} \sim \text{Binomial}(A_{\text{total}, t}, \lambda),
$$
