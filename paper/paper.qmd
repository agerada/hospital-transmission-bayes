...
#| echo: false
#| fig.cap: "System dynamics model. $S$ and $I$ are the number of susceptible and infected patients, respectively; $A$ and $D$ are the rates of admitted and discharged patients, respectively; $\\lambda$ is the community infection rate; $\\beta$ is the transmission coefficient; and $r$ is the rate of random (*de novo*) infection."
...
A similar approach was used to generate outbreaks in the stochastic system dynamics model. To align with the agent-based model, parameters were changed at four discrete time points: 1) onset of outbreak; 2) start of enhanced IPC measures; 3) end of outbreak; and 4) end of enhanced \acr{IPC} measures. Three parameter regimes were used: baseline (pre- and post-outbreak), outbreak (pre-control), and control (during enhanced IPC). During the control period only the transmission coefficient $\\beta$ changed; the community infection rate $\\lambda$ was unaffected by control measures (it only differed between baseline and outbreak). The parameter $r$ (rate of random infection) was kept constant throughout the simulation.
...
Firstly, $n$ independent random samples were drawn from the parameter prior distributions giving $\theta_1,\theta_2,\ldots,\theta_n$. Next, each sample parameter point $\theta_i$ was used as an input to the agent-based simulation model, generating a total of $n$ simulated datasets. The number of daily cases of nosocomial \acr{CDI} were extracted from the model outputs, and converted to a time series of nosocomial \acr{CDI} per 1000 patient days for each month, generating simulated summary statistics $S_1,S_2,\ldots,S_n$. Since each simulation starts with an empty hospital that is gradually populated with patient admissions, results from the first 31 simulated days were discarded to ensure that the model had reached a steady state.
...
A similar calibration approach using \acr{ABC} was also used to calibrate the stochastic \acr{SD} model to the observed outbreak data, although the calibration was performed in a single step that encompassed the entire period of the observed data (i.e., pre-outbreak, outbreak, and control periods).
...
```{r fig-abc-sd-posteriors, include=TRUE, dev='png'}
#| echo: false
#| out.height: "400px"
