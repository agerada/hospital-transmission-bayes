...
Computer simulation approaches such as agent-based modelling (ABM) can be used to simulate the spread of infectious diseases by constructing a virtual environment (such as a hospital), within which agents (such as patients and/or staff) interact. Agent-based modelling has been used to model various infectious diseases, such as the transmission of important nosocomial pathogens such as methicillin-resistant _Staphylococcus aureus_ (MRSA) within a healthcare setting [@jaramilloAgentBasedModel2015], and the spread of SARS-CoV-2 within a community population [@kerrCovasimAgentbasedModel2021]. An important advantage of ABM is that models can be used to project the impact of interventions on transmission, prior to their implementation [@cassidyMathematicalModellingHealth2019]. This can be particularly useful in the preparedness for, and management of, outbreaks of healthcare-associated infections, where the impact of interventions is difficult to quantify [@cassidyMathematicalModellingHealth2019]. "What if?" testing allows for different scenarios or bundles of interventions can be simulated to determine their impact. 

In this study, we describe an agent-based model that simulates the transmission of faecal-oral-transmitted pathogens under antimicrobial selection pressure. The aim of the model is to guide infection control strategies and interventions, particularly during outbreak periods. The parameters are estimated using data from a CDI outbreak reported by @salgadoAnalysisOutbreakClostridium2009, although the model is designed to be adaptable to other related antimicrobial-resistant pathogens such as VRE, ESBL, and CPE.
...
Due to the inherent stochasticity within the agent-based model, all calibration and simulation runs were aggregated over multiple runs with different random number generators, and the mean rates of infection/colonisation were calculated. For the samples used for ABC, `r results_outbreak_control_n_seeds` different random number generator seeds were used;
...
One of the simulation runs was compared to the deterministic model, which was numerically integrated using Euler's method. Since the agent-based model was stochastic, all simulations were aggregated
...
#| echo: false
#| fig-cap: "Sensitivity analysis of the model to changes in key infection control parameters. The y axis shows the total number of nosocomial infections during the outbreak period."
