...
-   $T_i(t)$ is the treatment multiplier that depends on antimicrobial therapy and is defined as:
    ...
    $$
    T_i(t) = \begin{cases}
      m & \text{if patient $i$ on antimicrobial therapy at time $t$} \\
      1 & \text{otherwise, and}
    \end{cases}
    $$
...
For each $\theta_i$, its simulated summary statistic $S_i$ was compared to the summary statistic generated from the observed data, $S_\text{obs}$, and $\theta_i$ was was accepted as a sample from the approximate posterior distribution if:
