...
-   $p$ is the community colonisatio rate,
...
![System dynamics model](images/sd_model.svg)
...
list(
  ...
  mutate(
    ...
    param = case_when(
      ...
      param == "c-proportion-redistributed"     ~ "Proportion redistributed",
      param == "c-toilet-cleaning-rate"         ~ "Toilet cleaning rate",
  ...
  ggh4x::facetted_pos_scales(
    x = list(
      ...
      param %in% c("Antimicrobial prescription rate", "Proportion redistributed", "Toilet cleaning effect") ~
        scale_x_continuous(
...
```{r}
stochastic_sd <- read_csv("../out/sd_stochastic experiment-table.csv", skip = 6)
```
...
abc_sd <- abc(salgado$rates,
              sd_param_grid, dplyr::select(sd_sumstats, -param_row), tol=0.0025, method='rejection')
...
posterior_predictive_sim_sd <- 
  pmap_dfr(posterior_check_params,
        \(init_population, A, r, baseline_beta, baseline_p, outbreak_beta, outbreak_p, outbreak_start, outbreak_end, n_days, param_row, seed) {
            set.seed(seed)
            sd_stochastic(init_population, A, r, baseline_beta, baseline_p, outbreak_beta, outbreak_p, outbreak_start, outbreak_end, n_days) %>% 
              mutate(`random-seed` = seed,
                     param_row = param_row) %>% 
...
```{r fig-single-outbreak-deterministic-one-plot, include=FALSE}
#| echo: false
#| message: false
...
```{r fig-single-outbreak-deterministic-faceted, include=TRUE}
#| echo: false
#| message: false
...
```{r fig-abc-sd-posterior-predictive-check, include=TRUE}
#| echo: false
...
posterior_predictive_sim_sd_rates %>% 
  pivot_wider(names_from = date_sim, values_from = rate) %>% 
  pivot_longer(cols = !c(`random-seed`, `param_row`), names_to = "date_sim", values_to = "rate") %>% 
  mutate(date_sim = ymd(date_sim)) %>% 
  group_by(date_sim) %>%
  summarise(m = median(rate),
            q_high = quantile(rate, 0.75),
