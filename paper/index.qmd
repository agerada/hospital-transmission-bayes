...
$$
\frac{dH_S}{dt} = A_S - Q\frac{H_S(t)}{H_I(t)} - DH_S(t)
$$

$$
\frac{dH_I}{dt} = A_I + Q\frac{H_S(t)}{H_I(t)} - DH_I(t)
$$
...
Rejection sampling approximate Bayesian computation was employed to calibrate the model to the outbreak data [@sunnåker2013]. Priors were determined from domain knowledge or extracted from existing literature. The parameters in @tbl-consts were kept constant during the calibration process.
...
single_seeds <- single_outbreak_sim$`random-seed`
single_outbreak_one_seed <- single_outbreak_sim_mod %>% 
  filter(`random-seed` == single_seeds[1])
...
baselinerate <- seq(from=0.005, to=0.1, length.out = 100)
outbreakrate <- seq(0.005, 0.1, length.out = 100)
baselinecolonisation <- seq(0.9, 0.99, length.out = 100)
outbreakcolonisation <- seq(0.9, 0.99, length.out = 100)
...
# Initial conditions
initial_state <- c(N = 600, I = 17)  # Initial populations of non-infected and infected patients
time <- seq(0, run_to, by = 1)         # Time steps
dt <- 1                             # Time step size

plan(multisession)
ode_sims <- furrr::future_pmap_dfr(rate_grid,
        \(b_base, b_out, c_base, c_out) {
          ...
          for (t in 1:(length(time) - 1)) {
            ...
            dN <- p_t * parameters$A - parameters$A * prop_N - beta_t * N[t] * prop_I
            dI <- (1 - p_t) * parameters$A - parameters$A * prop_I + beta_t * N[t] * prop_I
...
abc(c(target_baseline_rate, target_outbreak_rate), rate_grid, ode_sims[c("baseline", "outbreak")], tol=0.5, method='rejection') %>% 
  summary()
...
baselinecolonisation <- 0.9721
outbreakcolonisation <- 0.9403
