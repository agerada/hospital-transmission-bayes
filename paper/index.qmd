...
# 
# baselinerate <- seq(from=0.01, to=0.99, length.out = 5)
# outbreakrate <- seq(0.01, 0.99, length.out = 5)
# rate_grid <- expand.grid(baselinerate, outbreakrate)
...
# Initial conditions
initial_state <- c(N = 600, I = 71)  # Initial populations of non-infected and infected patients
time <- seq(0, run_to, by = 1)         # Time steps
dt <- 1                             # Time step size
# 
# plan(multisession)
# furrr::future_map2(rate_grid$Var1, rate_grid$Var2,
#         \(x, y) {
#           print("1")
...
#             # Current beta value
#             beta_t <- beta_function(time[t], x, y)
#             
...
#             dN <- parameters$p * parameters$A - parameters$A * prop_N - beta_t * N[t] * prop_I
#             dI <- (1 - parameters$p) * parameters$A - parameters$A * prop_I + beta_t * N[t] * prop_I
#             
#             # Update populations using Euler's method
#             N[t + 1] <- N[t] + dN * dt
#             I[t + 1] <- I[t] + dI * dt
#           }
#           I[31:length(I)] / ( N[31:length(N)] + I[31:length(I)] )
#         })
